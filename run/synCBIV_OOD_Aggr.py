    ...
    for exp in range(args.num_reps):
        ...
        for r in br:
            ...
            if r in args.env_list:
                ...
                # 合并数据集
                combined_train = pd.concat([combined_train, train_df], ignore_index=True)
                combined_train = shuffle(combined_train)
                # combined_val = pd.concat([combined_val, val_df], ignore_index=True)
                # combined_val = shuffle(combined_val)
                # combined_test = pd.concat([combined_test, test_df], ignore_index=True)
