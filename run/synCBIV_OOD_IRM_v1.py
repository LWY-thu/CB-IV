...
from utils import *
import time
from module.SynCBIV_IRM import run as run_SynCBIV
...
def run(args):   
    ...
    for exp in range(args.num_reps):
        ...
        tr1, val1, te1 = split_data(train_df1_sample)
        tr2, val2, te2 = split_data(train_df2_sample)

        train1 = CausalDataset(tr1, variables = ['u','x','v','xs','z','p','s','m','t','g','y','f','c'], observe_vars=['v','x','xs'])
        val1 = CausalDataset(val1, variables = ['u','x','v','xs','z','p','s','m','t','g','y','f','c'], observe_vars=['v','x','xs'])
        test1 = CausalDataset(te1, variables = ['u','x','v','xs','z','p','s','m','t','g','y','f','c'], observe_vars=['v','x','xs'])
        train2 = CausalDataset(tr2, variables = ['u','x','v','xs','z','p','s','m','t','g','y','f','c'], observe_vars=['v','x','xs'])
        val2 = CausalDataset(val2, variables = ['u','x','v','xs','z','p','s','m','t','g','y','f','c'], observe_vars=['v','x','xs'])
        test2 = CausalDataset(te2, variables = ['u','x','v','xs','z','p','s','m','t','g','y','f','c'], observe_vars=['v','x','xs'])
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--env_str',default='[3.0, -1.5]',type=str,help='The environment list')
