...
import time
from module.Regression_OOD import run_ood_stage1 as run_Reg
...
class CBIV(object):
    ...
    def _build_graph(self, x, s, t, y_ , p_t, FLAGS, r_alpha, r_lambda, do_in, do_out, dims):
        ...
        # IRM 此处加入grad(loss)作为惩罚项
        # Q:for e in e_list的循环应该在表征层前还是表征层后
...
def trainNet(Net, sess, train_step, train_data, val_data, test_data, FLAGS, logfile, _logfile, exp, dataDir, resultDir, device, args, ood_test_dict):
    ...
    for i in range(FLAGS.iterations):
        ...
        if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
            ...
            # IRM output阶段 train_data用所有环境的合集
            ...
            if FLAGS.oodtestall == 1:
                for r in br:
                    ...
                    test = {'x':np.concatenate((test.x, test.xs), 1),
                            ...
                            'yf':test.y,
                            'ycf':test.f}
                    y_pred_f_test_tmp = sess.run(Net.output, feed_dict={Net.x: test['x'], Net.s: test['s'], Net.t: test['t'], Net.do_in: 1.0, Net.do_out: 1.0})
                    y_pred_cf_test_tmp = sess.run(Net.output, feed_dict={Net.x: test['x'], Net.s: 1-test['s'], Net.t: 1-test['t'], Net.do_in: 1.0, Net.do_out: 1.0})
                    y_pred_mu0_test_tmp = sess.run(Net.output, feed_dict={Net.x: test['x'], Net.s: test['s']-test['s'], Net.t: test['t']-test['t'], Net.do_in: 1.0, Net.do_out: 1.0})
                    y_pred_mu1_test_tmp = sess.run(Net.output, feed_dict={Net.x: test['x'], Net.s: 1-test['s']+test['s'], Net.t: 1-test['t']+test['t'], Net.do_in: 1.0, Net.do_out: 1.0})
                    ood_ate_test.append(np.mean(y_pred_mu1_test_tmp) - np.mean(y_pred_mu0_test_tmp))
...
def run(exp, args, dataDir, resultDir, train, val, test, device):
    ...
    alpha, lamda = args.syn_alpha, args.syn_lambda
    ...
    # 配置CPU选项
    # cpu_options = tf.compat.v1.ConfigProto(device_count={'GPU': 0})
